import theceloRedis from "./thecelo.redis.js";
import theceloUtils from "./thecelo.utils";

import { groups_key, eth_blockdata_key, validators_key, users_key, eth_dataset_key, election_current_key } from "./thecelo.const";
const bi23 = require('./celo.bi23.data');
export { groups_key, voteds_key, votes_key, election_current_key, validators_key } from "./thecelo.const.js";
export {theceloUtils} from './thecelo.utils.js';
export {redis_client, redis_subscribe_client} from "./thecelo.redis";

//
//
export class TheCelo {
  get eth_blockdata() {
    return this.eth_blockdata;
  }

  //key:address  value:[name,website,keybase,twitter]
  get users() {
    return this.users;
  }
  ...
  sync_datas() {
    theceloRedis.redis_client.get(eth_dataset_key, function (err, data) {
      if (!err && data) {
      }
    });
    theceloRedis.redis_client.get(users_key, function (err, data) {
      if (!err && data) {
  ...
  setSubscribe() {
    //
    theceloRedis.redis_subscribe_client.on("ready", function () {
      //subscribe messages
      theceloRedis.redis_subscribe_client.subscribe(eth_blockdata_key);
      theceloRedis.redis_subscribe_client.subscribe(groups_key);
      theceloRedis.redis_subscribe_client.subscribe(election_current_key);
      theceloRedis.redis_subscribe_client.subscribe(validators_key);
    });
    theceloRedis.redis_subscribe_client.on("subscribe", function (channel, count) {
      console.log(
        "client subscribed to " + channel + "," + count + " total subscriptions"
      );
    });
    //
    theceloRedis.redis_subscribe_client.on("message", function (channel, message) {
      console.log(channel + ":" + message);
      getSubscribeData(channel, message);
    });
    //
    theceloRedis.redis_subscribe_client.on("error", function (error) {
      console.log("redis_subscribe_client Error " + error);
    });
    //
    theceloRedis.redis_subscribe_client.on("unsubscribe", function (channel, count) {
      console.log(
  ...
  getSubscribeData(channel, message = "") {
    theceloUtils.log_out("getSubscribeData: " + channel);
    if (channel == groups_key) {
      theceloRedis.redis_client.get(groups_key, function (err, data) {
        if (!err && data) {
          ...
          Object.keys(groups).forEach((address, i) => {
            //
            theceloRedis.redis_client.get("group_" + address, function (err, data) {
              if (!err && data) {
                var obj = JSON.parse(data);
                var group = obj.group;
    ...
    //
    if (channel == election_current_key) {
      theceloRedis.redis_client.get(election_current_key, function (
        err,
        data
      ) {
        if (!err && data) {
    ...
    //
    if (channel == validators_key) {
      theceloRedis.redis_client.get(validators_key, function (err, data) {
        if (!err && data) {
          var obj = JSON.parse(data);
...
//         var data = await theceloRedis.get_redis_data('group_'+address);
...
//         theceloRedis.redis_client.get('validator_'+address, function(err, data) {
//           response.end(data);
//         });
//       }
...
//         theceloRedis.redis_client.get(theceloConst.accounts_key, function(err, data) {
//           var accounts = {};
//           var totalcGld = 0;
//           var totalcUSD = 0;
//           if(!err && data){
...
//         var data = await theceloRedis.get_redis_data('reserve_info');
//         response.end(data);
//       }
//       //////////////////////////////////////////////////////
//       else if(method=='exchange_records'){
//         let exchange_records = await theceloRedis.get_redis_data('celo_exchange_records');
//         response.end(exchange_records);
//       }
//       else if(method=='get_k_line'){
//         var k_time = params['k_time'];
//         let exchange_kline = await theceloRedis.get_redis_data('celo_exchange_kline_'+k_time);
...
//         let attestation_logs = await theceloRedis.get_redis_data('attestation_logs');
...
//         var data = await theceloRedis.get_redis_data('bittrex_day_prices');
//         response.end(data);
//       }
//       //
//       else if(method=='get_week_prices'){
//         var data = await theceloRedis.get_redis_data('bittrex_week_prices');
//         response.end(data);
//       }
//       //
...
//         //
//         var exchange_prices = [];
//         var exchange_data = await theceloRedis.get_redis_data('exchange_prices');
...
//         //
//         var bittrex_prices = [];
//         var bittrex_data = await theceloRedis.get_redis_data('bittrex_prices');
...
//           theceloRedis.redis_client.get(theceloConst.lockedGoldHistory_key, function(err, data) {
//             if(!err && data){
...
//           theceloRedis.redis_client.get(theceloConst.network_parameters_key, function(err, data) {
//             response.end(data);
//           });
//       }
//       else if(method == 'proposalList'){
//           theceloRedis.redis_client.get(theceloConst.proposalList_key, function(err, data) {
...
//         theceloRedis.redis_client.keys('*', function (err, keys) {
...
//         var key = params['key'];
//         theceloRedis.redis_client.get(key, function(err, data) {
//           if (err) return console.log(err);
//             response.end(data);
//         });
//       }
...
//           theceloRedis.redis_client.set(theceloConst.users_key,JSON.stringify(users));
//           //save logo image
...
//           theceloRedis.redis_client.get(theceloConst.users_key, function(err, data) {
...
//           theceloRedis.redis_client.get(theceloConst.voters_key, function(err, data) {
...
//           theceloRedis.redis_client.get(theceloConst.votes_key, function(err, data) {
...
//           redis_value = await theceloRedis.get_redis_data(key);
...
//           });
//           theceloRedis.redis_client.set(key,JSON.stringify(epochvotes));
//         }
//         response.end(JSON.stringify({epoch,epochvotes}));
//       }
//       //////////////////////////////////////////////////////
//       else if(method == 'latest_epoch_election_votes'){
//         let result = await theceloRedis.get_redis_data('latest_epoch_election_votes');
//         response.end(result);
//       }
//       //////////////////////////////////////////////////////
